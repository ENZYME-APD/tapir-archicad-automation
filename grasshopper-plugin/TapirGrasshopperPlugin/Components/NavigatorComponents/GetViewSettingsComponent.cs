...
namespace TapirGrasshopperPlugin.Components.ProjectComponents
{
    public class GetViewSettingsComponent : ArchicadAccessorComponent
    {
        ...
        {
            pManager.AddTextParameter("Json" + nameof(ViewSettings), "", "", GH_ParamAccess.item);
        }
        ...
        protected override void Solve (IGH_DataAccess DA)
        {
            ...
            if (!GetResponse(CommandName, inputObj, out JObject jObjectResponse)) return;

            var response = ViewSettingsResponse.FromResponse(jObjectResponse);

            DA.SetData(0, JsonConvert.SerializeObject (response, Formatting.Indented));
        }
