...
print ('generic keys found:', len (genericKeys) > 0)
propertyKeys = [k for k in genericKeys if k['key'].startswith ('PROPERTY-')]
print ('property-based keys found:', len (propertyKeys) > 0)
...
if walls:
    ...
    print ('wall has more keys than generic:', len (wallKeys) > len (genericKeys))
...
print ('PROJECTNAME resolves to its project info name:', names[0].get ('name') == projectNameField['projectInfoName'])
print ('unknown key is an error:', 'error' in names[1])
if propertyKeys:
    print ('property key resolves to its own name:', names[2].get ('name') == propertyKeys[0]['name'])
...
print ('embedded key resolved:', details['detailsOfElements'][0]['details']['text'] == 'Project: ' + projectNameField['projectInfoValue'])
