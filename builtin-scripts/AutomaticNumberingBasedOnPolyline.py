###############################################
# SCRIPT AUTOMATIC NUMBERING BASED ON POLYLINE V11
...
# Import tkinter for popup messages and input dialogs
import tkinter as tk
from tkinter import messagebox, simpledialog

# Function to show a popup message
def show_popup(message, title="Script Error"):
    root = tk.Tk()
...
# Name of the reference polyline object
referencePolylineName = 'AutomaticNumberingPolyline'

# Ask user for starting number
try:
    starting_number = show_input_dialog("Enter the starting number for the first element:", "Numbering Configuration", "1")
    ...
    if starting_number < 1:
        show_popup("Please enter a positive integer as the starting number.\nScript will now exit.", "Invalid Input")
        exit(1)
except ValueError:
    show_popup("Please enter a valid integer number.\nScript will now exit.", "Invalid Input")
    exit(1)
...
def generatePropertyValueString(elemIndex: int) -> str:
    return f"{elemIndex:03d}"  # Format as 001, 002, etc. without prefix
...
################################
# STEP 1: GET SELECTED ELEMENTS & FIND REFERENCE POLYLINE
################################
...
if not selectedElements:
    error_message = "No elements selected! Please select the reference polyline and the elements to number."
    show_popup(error_message)
    exit(1)

print(f"Found {len(selectedElements)} selected elements")

detailsOfElementsResponse = aclib.RunTapirCommand('GetDetailsOfElements', {'elements': selectedElements})
...
polyline_points = [(coord['x'], coord['y']) for coord in detailsOfReferencePolyline['details']['coordinates']]
    
print(f"Reference polyline has {len(polyline_points)} points")

if len(polyline_points) < 2:
    print("Warning: Reference polyline needs at least 2 points for proper numbering!")
    if len(polyline_points) == 0:
        print("No points found in the polyline.")
        exit(1)

################################
# STEP 2 & 3: GET ELEMENTS TO NUMBER
################################

# Exclude the reference polyline from the elements to number (if it's selected)
elementsToNumber = []
...
################################
# STEP 4: GET BOUNDING BOXES OF ELEMENTS TO NUMBER
################################
...
if not element_centers:
    print("Error: Could not calculate centers for any elements!")
    exit(1)

################################
# STEP 5 & 6: FIND CLOSEST POINTS AND SORT ELEMENTS
################################
...
for elem in element_centers:
    ...
    closest_point, min_distance, position_on_polyline = closest_point_on_polyline(polyline_points, center)
...
try:
    ...
    print(f"Successfully numbered {len(sorted_elements)} elements starting from {starting_number} according to their position along the reference polyline")
except Exception as e:
    print(f"Error setting property values: {e}")
    exit(1)

# Print the exact ordering for debugging
for index, elem in enumerate(sorted_elements, start=starting_number):
    print(f"Element {index}: {elem['element']['elementId']['guid']} at position {elem['position']:.2f}")

# Show success message to user
show_popup(f"Successfully numbered {len(sorted_elements)} elements starting from {starting_number}.", "Operation Complete")
